import { writeBatch, arrayRemove, arrayUnion, doc, collection, getDocs, limit, query, where, updateDoc, deleteField, Firestore } from "firebase/firestore"
import { ClassIdentifier } from "../class/util.ts";

const getTeacherUid = async (firestore: Firestore, email: string) => {
...
const getInvitation = ({hostEmail, classIdentifier, className}: GetInvitationArgs) => {
    const { classId, classGroupId } = classIdentifier
    const invitation: Invitation = {
        meta: {
            metaId: classId
        }
    }
    invitation[`invitations.${classId}`] = { // utilizing indexer
        hostEmail,
        className,
        status: true,
        classGroupId,
...
type MakeInviteDBArgs = GetInvitationArgs & {
    firestore: Firestore,
    recepientId: string,
    recepientEmail: string
}
const makeInviteDB = ({
    firestore, classIdentifier, recepientId, recepientEmail, hostEmail, className
}: MakeInviteDBArgs) => {
    const { classId, classGroupId } = classIdentifier
    const batch = writeBatch(firestore)
    batch.update(doc(firestore, "teachers", recepientId), getInvitation({classIdentifier, hostEmail, className}))
...
type RemoveInviteDBArgs = Pick<MakeInviteDBArgs, "firestore" | "classIdentifier" | "recepientId">
const removeInviteDB = ({
...
export { makeInviteDB, removeInviteDB }
